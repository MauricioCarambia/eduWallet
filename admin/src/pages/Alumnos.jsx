...
const verQR = async (alumno) => {
  ...
  const abrirEditar = (a) => {
    ...
    setForm({
      ...
      alergias: a.alergias || "Ninguna",
    });
    setModal("editar");
  };
  const abrirHistorial = (a) => {
    setSeleccionado(a);
    setModal("historial");
  };
  const abrirRecargar = (a) => {
    setSeleccionado(a);
    setMontoRecarga("");
    setModal("recargar");
  };
  ...
  const guardarNuevo = async () => {
    ...
    try {
      ...
      showMsg("error", "Error al registrar");
    }
  };
  ...
  const guardarEditar = async () => {
    try {
      ...
      setAlumnos((prev) =>
        prev.map((a) => (a.id === seleccionado.id ? res.data : a)),
      );
  ...
  const recargar = async () => {
    ...
    if (!n || n <= 0) return;
    try {
      const res = await api.post(`/alumnos/${seleccionado.id}/recargar`, {
        monto: n,
        empleado_id: 1,
        descripcion: "Recarga desde admin",
      });
      setAlumnos((prev) =>
        prev.map((a) => (a.id === seleccionado.id ? res.data : a)),
      );
      ...
      showMsg("error", "Error al recargar");
    }
  };

  const historialAlumno = seleccionado
    ? txs.filter((t) => t.alumno_id === seleccionado.id)
    : [];
  ...
  return (
    <div>
                ...
                <tr key={a.id} style={{ borderBottom: "1px solid #F8F9FA" }}>
                  ...
                  <td
                    style={{
                      ...
                      fontSize: 13,
                      fontWeight: 600,
                      color: parseFloat(a.saldo) < 200 ? "#DC2626" : "#111",
                    }}
                  >
                    {fmt(a.saldo)}
                  </td>
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                      {[
                        ...
                        {
                          label: a.activo ? "Bloquear" : "Activar",
                          onClick: () => toggleBloqueo(a),
                          color: a.activo ? "#FEF2F2" : "#F0FDF4",
                          textColor: a.activo ? "#DC2626" : "#16A34A",
                        },
                        {
                          label: "×",
      ...
      {(modal === "nuevo" || modal === "editar") && (
        <Modal
          title={modal === "nuevo" ? "Nuevo alumno" : "Editar alumno"}
          onClose={cerrarModal}
        >
          {[
            ...
            ["Tutor/Padre", "tutor", "text"],
            ["Teléfono tutor", "tutor_tel", "text"],
          ...
          >
            ...
            <Btn onClick={modal === "nuevo" ? guardarNuevo : guardarEditar}>
      ...
      {modal === "historial" && seleccionado && (
        <Modal
          title={`Historial — ${seleccionado.nombre}`}
          onClose={cerrarModal}
        >
          <div
            style={{
            ...
            >
              ...
              <p
                style={{
                  ...
                  fontWeight: 600,
                  color:
                    parseFloat(seleccionado.saldo) < 200 ? "#DC2626" : "#111",
                }}
              >
                {fmt(seleccionado.saldo)}
              </p>
            </div>
            <div
              style={{
              ...
              >
                ...
                <span
                  style={{
                    fontSize: 13,
                    fontWeight: 600,
                    color: t.tipo === "recarga" ? "#16A34A" : "#111",
                  }}
                >
                  {t.tipo === "recarga" ? "+" : "-"}
                  {fmt(t.monto)}
      ...
      {modal === "recargar" && seleccionado && (
        <Modal
          title={`Recargar — ${seleccionado.nombre}`}
          onClose={cerrarModal}
        >
          <p style={{ fontSize: 13, color: "#666", marginBottom: 16 }}>
            Saldo actual:{" "}
            <b style={{ color: "#111" }}>{fmt(seleccionado.saldo)}</b>
          </p>
            ...
            {[200, 500, 1000, 2000].map((n) => (
              <button
                ...
                style={{
                  flex: 1,
                  padding: "8px 4px",
                  border: `1px solid ${montoRecarga == n ? "#111" : "#F0F0F0"}`,
                  borderRadius: 8,
                  background: montoRecarga == n ? "#111" : "white",
                  color: montoRecarga == n ? "white" : "#666",
                  fontSize: 12,
                  cursor: "pointer",
                  fontWeight: montoRecarga == n ? 500 : 400,
                }}
              >
                {fmt(n)}
              </button>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            ...
            <Btn onClick={recargar} color="#16A34A">
              Recargar
            </Btn>
          </div>
        </Modal>
      ...
      {qrModal && (
        <Modal
          title={`QR — ${qrModal.alumno.nombre}`}
        ...
        >
          <div style={{ textAlign: "center" }}>
            ...
            <p style={{ margin: "10px 0 4px", fontSize: 13, color: "#666" }}>
              Código: <b>{qrModal.codigo}</b>
            </p>
            <p style={{ margin: "0 0 16px", fontSize: 12, color: "#999" }}>
              {qrModal.alumno.curso}
            </p>
            <button
              onClick={() => {
                const a = document.createElement("a");
                a.href = qrModal.qr;
                a.download = `QR-${qrModal.alumno.nombre}.png`;
                a.click();
              }}
              style={{
                padding: "8px 20px",
