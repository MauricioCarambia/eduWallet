...
export default function Venta() {
  ...
  if (cargando) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '80vh', color: '#999', fontSize: 14 }}>Cargando...</div>
  )
  ...
  if (!caja) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '80vh' }}>
      <div style={{ background: 'white', borderRadius: 20, padding: '2rem', width: '100%', maxWidth: 400, border: '1px solid #F0F0F0', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        ...
        <button onClick={handleAbrirCaja} style={{ width: '100%', padding: '16px', border: 'none', borderRadius: 12, background: '#16A34A', color: 'white', fontSize: 16, fontWeight: 600, cursor: 'pointer' }}>
          Abrir caja
        </button>
        {msg && <div style={{ marginTop: 12, padding: '8px 12px', borderRadius: 8, fontSize: 13, background: '#FEF2F2', color: '#DC2626' }}>{msg.texto}</div>}
      </div>
    </div>
  )
  ...
  if (vistaVentas) return (
    <div style={{ maxWidth: 600, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
        <button onClick={() => setVistaVentas(false)} style={{ background: 'none', border: 'none', fontSize: 20, color: '#666', cursor: 'pointer' }}>←</button>
        <h2 style={{ fontSize: 18, fontWeight: 600, margin: 0 }}>Ventas del turno</h2>
      </div>
      ...
      <button onClick={cargarVentasHoy} style={{ width: '100%', padding: '12px', border: '1px solid #F0F0F0', borderRadius: 10, background: 'white', fontSize: 14, color: '#666', marginBottom: 12, cursor: 'pointer' }}>
        Actualizar
      </button>
      <div style={{ background: 'white', borderRadius: 12, border: '1px solid #F0F0F0', overflow: 'hidden', marginBottom: 16 }}>
        {txsHoy.length === 0 ? (
      ...
      <button onClick={handleCerrarCaja} style={{ width: '100%', padding: '14px', border: 'none', borderRadius: 12, background: '#DC2626', color: 'white', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>
        Cerrar caja
      </button>
    </div>
  )
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 0, height: 'calc(100vh - 120px)', overflow: 'hidden', margin: '-24px', borderRadius: 0 }}>
      {/* toast */}
      {msg && (
        <div style={{ position: 'fixed', top: 20, left: '50%', transform: 'translateX(-50%)', zIndex: 100, padding: '10px 20px', borderRadius: 10, fontSize: 14, fontWeight: 500, background: msg.tipo === 'ok' ? '#111' : msg.tipo === 'warn' ? '#D97706' : '#DC2626', color: 'white', boxShadow: '0 4px 12px rgba(0,0,0,0.15)', whiteSpace: 'nowrap' }}>
          {msg.texto}
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: '1px solid #F0F0F0' }}>
        <div style={{ padding: '12px 16px', background: 'white', borderBottom: '1px solid #F0F0F0' }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
            {['Kiosco', 'Librería'].map(l => (
              <button key={l} onClick={() => { setLocal(l); setCarrito([]); setBusq('') }} style={{ flex: 1, padding: '10px', border: `2px solid ${local === l ? '#111' : '#F0F0F0'}`, borderRadius: 10, background: local === l ? '#111' : 'white', color: local === l ? 'white' : '#666', fontSize: 14, fontWeight: local === l ? 600 : 400, cursor: 'pointer' }}>{l}</button>
            ))}
            <button onClick={() => { setVistaVentas(true); cargarVentasHoy() }} title="Ver ventas del turno" style={{ padding: '10px 14px', border: '1px solid #F0F0F0', borderRadius: 10, background: 'white', color: '#666', fontSize: 13, cursor: 'pointer' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 10 }}>
            {prods.map(p => (
              <button key={p.id} onClick={() => addProd(p)} disabled={p.stock === 0}
                style={{ padding: '14px 12px', border: `1.5px solid ${p.stock === 0 ? '#FEE2E2' : '#F0F0F0'}`, borderRadius: 14, background: p.stock === 0 ? '#FFF5F5' : 'white', textAlign: 'left', opacity: p.stock === 0 ? 0.6 : 1, cursor: p.stock === 0 ? 'not-allowed' : 'pointer' }}>
      ...
      {/* panel derecho */}
      <div style={{ display: 'flex', flexDirection: 'column', background: 'white', overflow: 'hidden', borderLeft: '1px solid #F0F0F0' }}>
        {/* info caja */}
        <div style={{ padding: '8px 14px', background: '#F0FDF4', borderBottom: '1px solid #D1FAE5', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 12, color: '#166534', fontWeight: 500 }}>{caja.local} · {fmt(caja.ventas || 0)}</span>
          <button onClick={handleCerrarCaja} style={{ fontSize: 11, padding: '3px 10px', border: 'none', borderRadius: 6, background: '#DC2626', color: 'white', cursor: 'pointer' }}>Cerrar caja</button>
        </div>
          ...
          {alumno ? (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ margin: 0, fontSize: 13, fontWeight: 600 }}>{alumno.nombre}</p>
                  <p style={{ margin: 0, fontSize: 11, color: '#999' }}>{alumno.curso}</p>
                </div>
                <button onClick={() => { setAlumno(null); setCarrito([]) }} style={{ background: 'none', border: 'none', fontSize: 20, color: '#999', cursor: 'pointer' }}>×</button>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                <div style={{ background: parseFloat(alumno.saldo) < 200 ? '#FEF2F2' : '#F0FDF4', borderRadius: 8, padding: '6px 10px' }}>
                  <p style={{ margin: '0 0 1px', fontSize: 10, color: '#666' }}>Saldo</p>
                  <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: parseFloat(alumno.saldo) < 200 ? '#DC2626' : '#16A34A' }}>{fmt(alumno.saldo)}</p>
                </div>
                <div style={{ background: '#F8F9FA', borderRadius: 8, padding: '6px 10px' }}>
                  <p style={{ margin: '0 0 1px', fontSize: 10, color: '#666' }}>Gastado hoy</p>
                  <p style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>{fmt(alumno.gasto_hoy)}</p>
                </div>
              </div>
              {alumno.alergias !== 'Ninguna' && (
                <div style={{ marginTop: 6, padding: '6px 10px', background: '#FFF7ED', borderRadius: 7, fontSize: 12, color: '#D97706', fontWeight: 500 }}>⚠ Alergia: {alumno.alergias}</div>
              )}
            </div>
          ) : (
            <button onClick={simularScan} disabled={scanning} style={{ width: '100%', padding: '16px', border: '2px dashed #E5E7EB', borderRadius: 12, background: '#F8F9FA', fontSize: 13, color: '#999', fontWeight: 500, cursor: 'pointer' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '10px 14px' }}>
          {carrito.length === 0 ? (
            <p style={{ color: '#ccc', fontSize: 13, textAlign: 'center', marginTop: 16 }}>Sin productos</p>
          ) : carrito.map(i => (
            <div key={i.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 0', borderBottom: '1px solid #F8F9FA' }}>
              <span style={{ flex: 1, fontSize: 13, fontWeight: 500 }}>{i.nombre}</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <button onClick={() => remProd(i.id)} style={{ width: 24, height: 24, border: '1px solid #F0F0F0', borderRadius: 6, background: 'white', fontSize: 15, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#666', cursor: 'pointer' }}>−</button>
                <span style={{ fontSize: 13, fontWeight: 600, minWidth: 16, textAlign: 'center' }}>{i.qty}</span>
                <button onClick={() => addProd(i)} style={{ width: 24, height: 24, border: '1px solid #F0F0F0', borderRadius: 6, background: 'white', fontSize: 15, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#666', cursor: 'pointer' }}>+</button>
              </div>
              <span style={{ fontSize: 12, fontWeight: 600, minWidth: 60, textAlign: 'right' }}>{fmt(i.precio * i.qty)}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: '12px 14px', borderTop: '1px solid #F0F0F0' }}>
          {carrito.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              ...
              {descPct > 0 && <span style={{ fontSize: 11, color: '#16A34A' }}>−{fmt(total - totalDesc)}</span>}
            </div>
          )}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <span style={{ fontSize: 15, fontWeight: 600 }}>Total</span>
            <span style={{ fontSize: 22, fontWeight: 700 }}>{fmt(totalDesc)}</span>
          </div>
          <button onClick={cobrar} disabled={!alumno || carrito.length === 0 || procesando}
            style={{ width: '100%', padding: '14px', border: 'none', borderRadius: 12, background: !alumno || carrito.length === 0 ? '#F0F0F0' : '#111', color: !alumno || carrito.length === 0 ? '#999' : 'white', fontSize: 15, fontWeight: 700, cursor: !alumno || carrito.length === 0 ? 'not-allowed' : 'pointer' }}>
